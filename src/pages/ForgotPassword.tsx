...
export default function ForgotPasswordPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!email.trim()) {
      toast.error(t("forgot.emailRequiredTitle"), {
        description: t("forgot.emailRequiredDesc")
      });
      return;
    }

    if (!validateEmail(email)) {
      toast.error(t("forgot.invalidEmailTitle"), {
        description: t("forgot.invalidEmailDesc")
      });
      return;
    }
  ...
  return (
    <div className="min-h-screen bg-secondary flex flex-col items-center justify-center px-6">
      ...
      >
        ...
        <h1 className="text-2xl font-bold mb-2 font-lora">{t("forgot.title")}</h1>
        ...
        {sent ? (
          <div className="space-y-4">
            <div className="bg-eco-light rounded-lg p-4">
              <p className="text-sm text-eco font-medium">✓ {t("forgot.resetSent", { email })}</p>
            </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="text-left">
              <label className="text-sm font-semibold mb-2 block">{t("forgot.emailLabel")}</label>
            ...
            >
              {loading ? (
                ...
                <>{t("forgot.sendResetLink")} <ArrowRight className="w-4 h-4" /></>
              )}
            </button>
          </form>
        )}
        ...
        <div className="relative flex items-center justify-center my-6">
          ...
          <span className="mx-4 text-xs text-muted-foreground uppercase">{t("common.or")}</span>
          <div className="border-t border-border flex-1" />
        </div>

        <Link to="/login" className="text-sm text-muted-foreground hover:text-foreground flex items-center justify-center gap-1">
          <ChevronLeft className="w-4 h-4" /> {t("forgot.returnToLogin")}
        </Link>
      </motion.div>
